...
The debate about concealing the trace of texts written with artificial intelligence assistance requires a careful distinction — one that is easily misread. A text written with AI assistance is not a text written by AI. It is entirely legitimate for a researcher to express their own analysis more clearly with AI support. What is not legitimate is presenting content produced by artificial intelligence as one's own intellectual output.

This distinction explains why the concept of "anti-AI-trace writing" is ethically complex territory. When Else (2023) demonstrated that AI-generated abstracts could deceive experienced scientists, the problem that emerged was not the invisibility of the trace. It was the absence of disclosure. Whether a text carries an AI trace is a secondary question. The primary question is whether the AI contribution has been disclosed. This booklet is not a guide to hiding the trace. On the contrary: while preserving the obligation to disclose, it shows how AI assistance is honestly stated in a revision workflow. Integrity lies not in the visibility of the trace but in the transparency of the contribution.
...
Not every reviewer comment is correct or applicable. Some arise from a misreading of the manuscript. Some fall outside its stated scope. Some are mutually contradictory — one reviewer asks for a change while another requests the opposite. Each of these situations has a protocol.

The basic rule: no comment is passed over without being addressed. If the author decides not to apply a comment, that decision must be stated explicitly and justified. The justification must rest on the scientific argument, not on the reviewer's credibility. The formulation is not "the reviewer is mistaken" but "at this point the literature supports the following position." This is not merely a matter of tone. Williams and Bizup (2016), in their treatment of clarity and argumentation, note as a general principle that the strength of an argument lies in its evidence, not in how forcefully it is delivered — a principle that applies with particular force in a rebuttal context, where the reader already has reason to be skeptical. Even when a reviewer is wrong, the response stays respectful and evidence-anchored.

The editor escalation threshold is as follows. When two reviewers contradict each other directly, or when a comment conflicts with the fundamental design of the manuscript in a way the author cannot resolve unilaterally, the author writes a brief direct note to the editor flagging the conflict and requesting direction. This is safer — and more collegial — than attempting to resolve the contradiction alone and hoping the editor does not notice.
