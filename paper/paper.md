...
Twenty-one booklets are released at v3.0.0, and every one of the twelve categories carries at least one. Each booklet is short, opinionated, and tested in the author's own academic practice. The released catalog spans the four Foundations booklets, regional indexing, the Memory-as-Vault introduction and the source passport, folder discipline and the MOC pattern, ritual hooks for session automation, the Model Context Protocol for the researcher, IMRAD scaffolding, APA 7 with DOI discipline, journal fit and cover letters, reproducible quantitative workflows, statistical test selection, qualitative coding with AI assistance under human oversight, the ethics backbone, rebuttal letters with traceability matrices, anti-AI-trace writing for revisions, conference slides, posters, and lightning talks, and the closing troubleshooting protocol. Each booklet is assigned a stable immutable identifier of the form KKK-AA-SSSS that does not change after release even when the content is revised.

The v1.1.x release adds ten companion Claude Code project skills under `.claude/skills/`, providing compact, repeatable operating protocols for the most common workflows: literature triage, APA 7 and DOI verification, bilingual booklet pairing checks, AI disclosure auditing, ethics and IRB checklists, rebuttal traceability matrices, memory vault architecture, regional access routing, agentic session debugging, and repository release integrity checking. The v2.7.0 release doubles the set to twenty, extending coverage across the full research lifecycle with skills for anti-AI-trace revision, bilingual manuscript scaffolding, journal fit screening, qualitative coding discipline, statistical consultation, research ritual hooks, lifecycle routing, MCP stack triage, source passport ledgers, and bilingual conference materials. Every skill carries a mandatory Turkish usage section enforced by the repository validator, extending the guide's bilingual parity principle to the skill layer. The skills install through a pip package (`social-cc-plugin`) or through the Claude Code plugin system, ship with a `social-cc doctor` environment check, and are dual-licensed to match the repository.

# Distinctive Scholarly Contributions

Five features distinguish this resource from comparable guides and merit explicit description.
...
The fifth is a worked position on the ethics of AI writing style, the anti-AI-trace revision method. Empirical work shows that automated detectors of AI-generated text are unreliable, with the highest accuracies below 80 percent and sharp degradation under paraphrase [@weberwulff2023], and that they systematically misclassify the writing of non-native English speakers as machine output [@liang2023]. The guide draws the consequence explicitly: research integrity must rest on disclosure rather than on detectability. Booklet 010-02-0001 and the companion `anti-ai-trace-revision` skill teach a two-layer revision method, rhetorical de-patterning and bilingual naturalness restoration, under the fixed principle that removing stylistic traces is legitimate authorship while the disclosure obligation stays untouched. The method is applied reflexively to the guide's own prose, which has passed the same de-patterning revision it teaches.
...
The continuous integration workflow runs markdownlint across all Markdown files, the repository integrity validator (`scripts/validate-repo.mjs`), and the installer's Python test suite on Python 3.9 and 3.12 on every commit and pull request. The validator checks bilingual pairing, frontmatter schema completeness, fabricated citation counts, catalog consistency, and the eight-section schema of every companion skill, including the mandatory Turkish usage section. Any booklet committing to the main branch with a `pending` human review state or a non-zero fabricated citation count fails the CI check and is blocked from release.
