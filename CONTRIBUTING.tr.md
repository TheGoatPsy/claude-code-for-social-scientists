...
Sürdürücü katkıları ayda bir kez gözden geçirir ve yeni broşür önerilerini katalog içine yerleştirir.

## Başlamadan önce

Bir konu (issue) açın. Önerinizi iki ya da üç paragrafta tarif edin. Yeni bir broşür öneriyorsanız önerdiğiniz `KKK-AA-SSSS` kimliğini (uygun kategorideki bir sonraki boş seri), bir paragraf kapsamı ve bir taslak ana hattı ekleyin.

Bu adım on beş dakikanızı alır ve hem size hem de sürdürücüye uyumsuz iş saatleri kazandırır.

## İki dilli eşleşme kuralı

Her broşür klasörü `tr.md` ve `en.md` dosyalarını yan yana içerir. Bir sürekli entegrasyon kontrolü, `main` dalına yapılan her commit'te bunu zorunlu kılar.
...
Taslağında yapay zekâ kullanan her katkı sahibi, dokunduğu broşürün yapay zekâ ifşa frontmatter alanlarını doldurmalıdır. Şema [`AI-AUTHORSHIP.md`](./AI-AUTHORSHIP.md) içinde belgelenir. Asgari zorunlu alanlar şunlardır.
...
Bir atıfı genişletmek için yapay zekâ yardımı aldıysanız, yapay zekânın ilk yanıtı bir aday atıftır, atıf değildir. Her adayı bir indekse karşı çözün. Yazar ve sürdürücü, atıf uydurmasını sürüm engelleyici bir hata olarak değerlendirir.
...
Merge edilen dış katkı sahipleri katkı türü ile birlikte [`meta/contributors.md`](./meta/contributors.md) içinde listelenir.
